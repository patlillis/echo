...
gulp.task('html', () => {
  return gulp.src('src/html/**/*.html')
    ...
    .pipe(gulpif(!yargs.argv.dev, htmlmin({ collapseWhitespace: true, removeComments: true })))
    .pipe(gulp.dest('dist'))
})
...
const buildSass = () => {
  return gulp.src('src/sass/style.scss')
    ...
    .pipe(sass({ errorHandler: onError }))
    .pipe(autoprefixer({ browsers: [ 'last 2 versions', 'ie >= 9', 'Android >= 4.1' ] }))
    .pipe(gulpif(!yargs.argv.dev, nano({ safe: true })))
...
const build = () => {
  ...
  return bundler
    ...
    .pipe(gulpif(!yargs.argv.dev, uglify()))
    .pipe(sourcemaps.write('./maps', { addComment: false }))
    .pipe(gulp.dest('dist'))
    .pipe(sync.stream());
}
